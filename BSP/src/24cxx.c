...
uint8_t at24cxx_read_one_byte(uint16_t addr)
{
    ...
    {
        iic2_send_byte(0xA0);    /* 发送写命令, IIC规定最低位是0, 表示写入 */
        iic2_wait_ack();         /* 每次发送完一个字节,都要等待ACK */
        iic2_send_byte(addr >> 8);   /* 发送高字节地址 */
    }
    else 
    {
        iic2_send_byte(0xA0 + ((addr >> 8) << 1));   /* 发送器件 0xA0 + 高位a8/a9/a10地址,写数据 */
    }
    
    iic2_wait_ack();             /* 每次发送完一个字节,都要等待ACK */
    iic2_send_byte(addr % 256);  /* 发送低位地址 */
    iic2_wait_ack();             /* 等待ACK, 此时地址发送完成了 */
    
    iic2_start();                /* 重新发送起始信号 */ 
    iic2_send_byte(0xA1);        /* 进入接收模式, IIC规定最低位是1, 表示读取 */
    iic2_wait_ack();             /* 每次发送完一个字节,都要等待ACK */
    temp = iic2_read_byte(0);    /* 接收一个字节数据 */
    iic2_stop();                 /* 产生一个停止条件 */
    return temp;
}
...
void at24cxx_write_one_byte(uint16_t addr, uint8_t data)
{
    ...
    iic2_send_byte(data);        /* 发送1字节 */
    iic2_wait_ack();             /* 等待ACK */
    iic2_stop();                 /* 产生一个停止条件 */
    // tim_delay_us(10000);               /* 注意: EEPROM 写入比较慢,必须等到10ms后再写下一个字节 */
    HAL_Delay(10);
}
